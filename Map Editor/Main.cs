...
namespace Map_Editor
{
    public partial class Main : Form
    {
        ...
        private void UpdateEnviroment()
        {
            ...
            Text = string.Format("FPS: {0}---Map:W {1}:H {2} ----W,S,A,D,--Suprcode--v.1.2--<{3}>", FPS, mapWidth, mapHeight,
                mapFileName);
        }
        ...
        private void Main_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                ...
                case Keys.Divide:                           // Fully 'Zoom In' using NumPad '/' key (equivalent to Shift-Add)
                    zoomMIN = zoomMAX;
                    break;
                case Keys.Multiply:                         // Fully 'Zoom Out' using NumPad '*' key (equivalent to Shift-Subtract)
                    zoomMIN = 1;
                    break;
                ...
                case Keys.M:
                    if (e.Alt & e.Control)
                    {
                        createMapImage_XLarge();    // "MapImage_XLarge" is 4x MiniMap size
                        return;
                    }
                    else if (e.Alt)
                    {
                        createBigMap();        // "BigMap" is 1/2 MiniMap size
                        return;
                    }
                    else if (e.Control)
                    {
                        createMapImage_Large();        // "MapImage_Large" is 2x MiniMap size
                        return;
                    }
                    else
                    {
                        createMiniMap();            // Minimap is the 'standard' size (/4). Its size is btw BigMap and the L, XL MapImages.
                    }
                    break;
                case Keys.N:
                    NewMap();
        ...
        private void menu_DeleteSelectedBackLimitCellData_Click(object sender, EventArgs e)    //M2P
        {
            if (M2CellInfo != null)
            {
                DialogResult dr = MessageBox.Show("Are You Sure You Want to Delete Selected Back (Yellow) Limit Cell Data?" + "\n" + "The Operation Cannot Be UnDone!", "Delete", MessageBoxButtons.OKCancel);
        ...
        private void menu_DeleteSelectedFrontLimitCellData_Click(object sender, EventArgs e)    //M2P
        {
            if (M2CellInfo != null)
            {
                DialogResult dr = MessageBox.Show("Are You Sure You Want to Delete Selected Front (Red) Limit Cell Data?" + "\n" + "The Operation Cannot Be UnDone!", "Delete", MessageBoxButtons.OKCancel);
        ...
        public void createBigMap()
        {
            ...
            miniBitmap = new Bitmap(miniBitmap, (miniBitmap.Width / 8), (miniBitmap.Height / 8));  //Standard for MiniMap is /4, 'BigMap' is /8 to reduce map size for maps ~700-800 sq tiles and up.
        ...
        public void createMapImage_Large()
        {
            Bitmap miniBitmap = new Bitmap(mapWidth * 12, mapHeight * 8, System.Drawing.Imaging.PixelFormat.Format32bppArgb);
            ...
            miniBitmap = new Bitmap(miniBitmap, (miniBitmap.Width / 2), (miniBitmap.Height / 2)); //Standard for MiniMap is /4, 'Large' MapImage is /2 for maps btw ~ 300sq & 600sq tiles.

            string SimpleFileName = Path.GetDirectoryName(mapFileName) + @"\" + Path.GetFileNameWithoutExtension(mapFileName);
            //MessageBox.Show(SimpleFileName);
            miniBitmap.Save(SimpleFileName + "_MapImage_L.png", ImageFormat.Png);
            MessageBox.Show("Saved... " + SimpleFileName + "_MapImage_L.png");
        ...
        public void createMapImage_XLarge()
        {
            Bitmap miniBitmap = new Bitmap(mapWidth * 12, mapHeight * 8, System.Drawing.Imaging.PixelFormat.Format32bppArgb);
            ...
            miniBitmap = new Bitmap(miniBitmap, (miniBitmap.Width / 1), (miniBitmap.Height / 1)); //Standard for MiniMap is /4, 'XLarge' MapImage is /1 for maps btw ~ 150sq & 500sq tiles.

            string SimpleFileName = Path.GetDirectoryName(mapFileName) + @"\" + Path.GetFileNameWithoutExtension(mapFileName);
            //MessageBox.Show(SimpleFileName);
            miniBitmap.Save(SimpleFileName + "_MapImage_XL.png", ImageFormat.Png);
            MessageBox.Show("Saved... " + SimpleFileName + "_MapImage_XL.png");
